...
def _upload_to_s3(file_obj: BinaryIO, file_type: str) -> str:
    ...
    try:
        ...
        logger.error(
            f"❌ S3 upload failed: {type(e).__name__}: {str(e)}", exc_info=True
        )
        raise Exception(f"S3 upload failed: {str(e)}")


async def upload(file_path: BinaryIO, file_type: str) -> str:
    """Async wrapper for S3 upload."""

    S3_url = await run_in_threadpool(_upload_to_s3, file_path, file_type)
    return S3_url
